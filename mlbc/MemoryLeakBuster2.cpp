...
void HookHeapAlloc()
{
	...
	if (!EnumProcessModules(hProcess, moduleHandles, sizeof(moduleHandles), &moduleSize)) {
		MessageBoxA(0, "dont get process modules", "Error", 0);
		return;
	}
	auto moduleNumber = moduleSize / sizeof(HMODULE);

	for (int index = 0; index < (int)moduleNumber; index++) {
		GetModuleBaseNameA(hProcess, moduleHandles[index], basename, MAX_PATH);
		GetModuleFileNameA(moduleHandles[index], filename, MAX_PATH);

		fprintf(stdout, "base : %s file : %s \n", basename, filename);

		EachImportFunction(::GetModuleHandleA(basename), "kernel32.dll", [](const char *funcname, void *&imp_func) {
